...
type MapReduce struct {
	...
	jobNum *JobCounter // current job number
	opCount *JobCounter
	finishedMap chan bool
	finishedReduce chan bool
	finishedAll bool
	finished chan bool
	writeMap chan bool

	JobTable *JobInfo
	// JobsInProgress *JobStack
	// JobsCompleted *JobStack
}

func (c *JobCounter) Inc() {
	c.Lock()
	c.counter++
	c.Unlock()
}

func (c *JobCounter) Reset() {
	// fmt.Println("LOCKING COUNTER")
	c.Lock()
	c.counter = 0
	c.Unlock()
	// fmt.Println("UNLOCKING COUNTER")
}

func (t *JobInfo) Init(numOfJobs int) {
	// fmt.Println("LOCKING TABLE")
	t.Lock()
	...
	t.Unlock()
	// fmt.Println("UNLOCKING TABLE")
}

func (t *JobInfo) ChangeStatus(jobNum int, status int) {
	t.Lock()
	t.table[jobNum] = status
	t.Unlock()
}
...
func InitMapReduce(nmap int, nreduce int,
	...
	mr.finishedAll = false
	mr.finished = make(chan bool)
	mr.finishedMap = make(chan bool)
	mr.finishedReduce = make(chan bool)
	mr.writeMap = make(chan bool)
	mr.opCount = &JobCounter{counter: 0}
	mr.JobTable = &JobInfo{}
